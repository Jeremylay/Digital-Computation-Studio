...
// CODED BY: MARK DEMERS.
//  ADJUSTED TO ADAPT TO ARDUINO CODE.
...
int Bauds[] = {9600, 19200, 31250, 38400, 57600};
...
void setup() {
  ...
  arduino = new Arduino(this, ports[SerialPortID], Bauds[SerialPortBaudRate]);
...
fill(93,132,245);
  ...
  myValue = arduino.analogRead(4);
  
    if (myValue > 50){
  fill(93,132,245,120);
  rect(random(500), random(800), 5, 55);
  }
...
void updatedisplay()
{
  ...
  if (state == 0)
  {
    ...
    text("Serial RX", 400-85, height-25);
    text("Serial TX", 400-85, height-12);

    // RX Flash square
    rect(400-30, height-34, 8, 8);

    // TX Flash square
    rect(400-30, height-21, 8, 8);
  }
  ...
  fill(255);
  textSize(12);
  text("Serial <> MIDI Converter", (400/2)-(300/2), 10, 300, 20);
  textSize(12);
  text("By Mark Demers - SpikenzieLabs.com", (400/2)-(300/2), 35, 300, 45);
  textAlign(LEFT);
  textSize(12);
}
